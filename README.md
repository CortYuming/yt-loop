...
- **🎼 Sheet** — a sheet per video: the fingerings you picked in [Guitar Chord Viewer](https://cortyuming.github.io/guitar-chord-viewer/), drawn as diagrams of the five frets around the shape and laid out as one unbroken row that slides under a playhead fixed in the middle — what is playing stays in one place and what is coming arrives from the right. Every bar is four slots wide whatever it holds, its chords taking slots in proportion to their beats, so the sheet moves at one steady pace; a bar with more than four chords stacks them four to a line inside that same width. The slot narrows on a window too small for four, so a whole bar always fits across. Seconds the sheet says nothing about are drawn as blank of the same width those seconds would have taken, so the row keeps moving through a hole instead of parking under the playhead; where two phrases overlap it never runs backwards — a bar starting before the sheet has got there is crossed at once. An amber rule and the bar number mark where each bar starts, and the number is also the way there: click it and the video goes to where that bar begins — reading a transcription is jumping about in it. Past a dozen bars the number is also faster typed than found: **Bar** at the end of the toolbar takes one and goes there. The time on a bar head is also how a range is marked out from the sheet: click it and **start📍 / end📍** appear, putting that bar's two edges in the boxes they name — one bar's start, a later bar's end, instead of catching both by ear as they go past. end📍 takes the bar's end rather than its start, so the bar it is clicked on is played through: bar 5's start📍 and bar 9's end📍 loop bars 5 to 9, and one bar on its own is its own time and both pins. A bar whose end nothing says — the last one, with no bar after it to borrow a length from — has no end to offer, and that pin says so rather than writing the start into **End**. The same panel holds the time itself, whether or not the editor is open: type it, or take it off the clock with **now📍**, and the bar line moves — the bar before ends where this one starts, so both sides of the line move together and nothing after them is touched. A head can only move between the head before it and its own end. Under the tab every chord is written again as a column of coloured dots, one to a string that rings, stacked highest first and hung from a fixed top so the voice the ear follows sits on one level however deep the chord under it is; a knocked string has no pitch and gets none. They are labelled with degrees by default, note names on the **Interval / Note** switch. The diagrams themselves carry no words in their dots — the board is drawn small, and what it is for is the shape of the hand and where on the neck it goes, which is why the fret numbers over it are the largest type on it; the root is set apart, open strings are hollow, muted ones get `×`. Under the staff every chord is written again as its degree of the key — a roman numeral, lower case for a minor third, the degree alone with no seventh or alteration on it, since the chord's own name above the staff already carries those. It reads as the number system a tune is transposed and remembered in, and it costs no height: the room below the staff is the stems' clearance, which is held open whatever the sheet holds. A sheet with no key named has nothing to count from and gets none. A chord name opens the full fretboard in the viewer, and the **↗** beside the key opens the key itself as one chord — every note of the scale in a single shape — labelled in solfège, which is the reading it is being looked at for. **Show** folds the strip away; 🎼 in a video's header loads that video and opens its editor. See the notation below
- **♪ Single notes** — the line actually played over a chord, not just the chord: a solo, a riff, the head of the tune. Written on the staff with the durations it is played in — stems, beams, flags, rests, dots and ties — and again below it as tab, so the same phrase says what the music is and where it is on the neck. A tab row appears only in a sheet that has notes in it. Where a note falls is never written down: a note follows the one before it and the durations stack up from the start of the chord's stretch, so re-timing one slides the rest along instead of leaving a row of positions to correct by hand. Notes struck together — a double stop, the three-note chords a chord-melody ends on — are one event with more than one string in it. A tie holds a note on without striking it again, which is also how a note carries over a bar line: the tie is simply the first thing in the next bar, and the pitch is not written twice. In 🎼 Edit mode every chord cell has a ♪ that opens the whole neck under the strip: pick a duration, tap a fret, and it is written. **A fingering is written on that same board** — click a chord where it is written on the staff and editing opens on that shape with stacking on, so each tap adds a string to it and a string already in it moves to where it was just tapped. Stacking is on from the first press because correcting a shape starts by tapping one of its strings, and with it off that tap threw the other five away; a single note opens replacing, as it always did. The diagram under the staff is the way out to Guitar Chord Viewer rather than the way in. Tapping a dot that is already lit takes that string off, which is the press that means “not this one”; a note left with no strings at all keeps its place and its length and becomes a rest. A chord still written as a name and six frets becomes the stop it sounds as the moment it is tapped. **None** is the duration a chord is written in: no length of its own, sounding until the next thing does. **Muted strings** — stopped with the left hand and struck anyway — are written with the ✕ button beside the rest and the tie. It is a mode rather than a mark on the note: while it is on, a fret tapped on the neck mutes that string of the note being edited, and tapping it again lets it ring. Which strings are muted is the question a chord asks, since its top voice usually rings over the courses the hand kills — the figure a comped bar of triplets is made of. A muted string is drawn as a cross where its head would be, and marked on the neck the same way. A **grace note** — drawn small with a stroke through its stem — is written with the button beside the rest and the tie: it is struck just before the note it leans on and takes no time from the bar, so the run around it is measured as if it were not there. **⧉ Copy** in the Fix row writes whatever is selected again just after it — a chord, a note, a rest — for the bar that holds one voicing while the tune moves. The panel's buttons stand in three rows by what they do: **Length**, **Write**, **Fix**. **↺ Undo** in the Fix row takes back one tap at a time (the Versions list files a run of them as one entry, which is what makes it readable), and the **Int / Note / Sol** switch at the end of that row labels the board's dots — the same choice as the pill above the strip. Clicking a note in the strip — head, fret number, or the column between them — selects it, and the same buttons then re-time it, dot it, move it to another string, or turn it into a rest or a tie. Walking the selection with **← →** brings the row with it: a note stepped past either edge slides the row back just far enough to show it and no further, so a phrase written across several bars is written without reaching for the strip. Only while the video is stopped — playing, the row is following the music, and the caret leaves it to. The dots on the board are labelled by the same **Interval / Note / Solfege** switch the row under the tab uses; the syllables are written a letter at a time — `d r m f s l t`, and two letters for an altered one (`ro`, `mo`, `sw`) — so the length of a label says whether the note is bent, and a spelled-out `swo` never has to be squeezed into a dot. Keys while the panel is open: **← →** to walk the selection, **1**–**5** for the durations, **0** no length, **.** dot, **R** rest, **T** tie, **G** grace note, **X** mute the strings you tap, **S** stack, **Backspace** delete, **Esc** out
- **Start after End** — not corrected, not allowed in. A Start later than the End describes nothing, so the value that would do it is refused at the door: 📍, a bar head's start📍 / end📍, typing, and `Shift + ← →` all leave the field as it was. A refused box flashes red, where an accepted one flashes white; a refused bar head says which end it is on the wrong side of, in the panel it was pressed in. Nothing is swapped or cleared behind your back — the pair you had is the pair you keep. A link written by hand can still carry the two the wrong way round, and then **▶ Play** with looping on, and both copy buttons, read `⚠ Start > End` for a moment and do nothing
...
- **📝 MD** — copies the same link as a Markdown link, labelled with the video title, the time range and — where the video has a sheet — which of its bars the range covers. A passage is remembered as "bars 5-8", not as a pair of times, so that is what the label carries. A bar counts as covered when it sounds inside the range: bar 5's time for the start and bar 9's for the end plays bars 5 to 8
...
| `2/6+3/6x+4/6x` | An `x` on a string is muted: that string stopped with the left hand and struck anyway, a knock rather than a pitch. Written per string, because that is how it is played — here the top voice rings over two muted courses, which is what a comped chord mostly is. Drawn as a cross where the head would be, in grey, a note with no pitch having no degree to be coloured by; the tab keeps the fret and greys it, the hand being where a player still has to put it. A whole shape knocked is every string of it crossed (`2/6x+3/6x+4/6x`) |
